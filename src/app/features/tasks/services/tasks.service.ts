...
export class TasksService extends ApiClientService {
  ...
  public getTasksList(userId: string): Observable<any> {
    ...
    return this.http
      ...
      .pipe(map(this.successBody), catchError(this.handleError));
  }

  public addTask(task: Task): Observable<any> {
    const url = `${this.urlBase}/tasks`;
    return this.http
      .post(url, task, { headers: this.getHeaders(), observe: 'response' })
      .pipe(map(this.successBody), catchError(this.handleError));
  }
  ...
  public editTask(taskId: string, task: Task): Observable<any> {
    ...
    return this.http
      .put(url, task, { headers: this.getHeaders(), observe: 'response' })
      .pipe(map(this.successBody), catchError(this.handleError));
  }
